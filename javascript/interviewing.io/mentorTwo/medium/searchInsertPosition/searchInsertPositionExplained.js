...
// Create a function that takes in an inputArray and a target
const searchInsertPosition = (inputArray, target) => {
  ...
  while (left <= right) {
    ...
    // If middle is the target than return it
    if (inputArray[middle] === target) {
      return middle
      // else if its lower than increment the left side
    } else if (inputArray[middle] < target) {
      left = middle + 1
      // Else decrement the right half
    } else {
      right = middle - 1
    }
  }
  // If it doesnt exist in the array return left which will be the index of where it should be
  return left
}
