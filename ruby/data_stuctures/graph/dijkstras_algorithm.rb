...
def dijkstra_shortest_path(starting_city, final_destination)
  ...
  # To keep our code simple we'll use a basic array to keep track of the known cities we havnt visited yet:
  unvisited_cities = []

  # We keep track of the cities we've visited using a hash table. We could have used an array, but since we'll be doing loopups
  # a hash table is more efficient:
  visited_cities = {}

  # We add the starting city's name as the first key inside the cheapest_prices_table. It has a value of 0,
  # since it costs nothing to get there:
  cheapest_prices_table[starting_city.name] = 0

  current_city = starting_city

  # This loop is the core of the algorithm. It runs as long as we can visit a city that we hanv't visited yet:
  while current_city
    # We add the current_city's name to the visited_cities hash to record that we've officially visited it. We also remove it from the list of unvisited cities:
    ...
    current_city.routes.each do |adjacent_city, price|
      ...
      # We caluclate the price of getting from the STARTING city to the ADJACENT city using the CURRENT city as the second to last stop:
    ...
    # We visit our next unvistied city. We choose the one that is cheapest to get to from the STARTING city:
    current_city = unvisited_cities.min do |city|
      cheapest_prices_table[city.name]
    end
  end

  # We have completed the core algorithm. At this point, the cheapest_price_table contains all the cheapest prices to get to each city
  # from the starting city. However, to calculate the precise path to take from our starting city to our final destination,
  # we need to move on.

  # We will build the shortest path using a simple array:
  shortest_path = []

  # To construct the shorest path, we need to work backwards from our final destination.
  ...
  # We reverse the output so we can see the path from the beginning to end:
